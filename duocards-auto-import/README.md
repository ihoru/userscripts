...
- Report a bug opens the GitHub template with the script version, available browser/manager versions, and page location prefilled. Missing versions can be filled in manually; query parameters, private path identifiers, and vocabulary are omitted. Review the form before submitting.
- During an automatic library Save, the exact native alert “That's already in the set.” is acknowledged without opening a dialog. The panel records Already in set, waits for automatic advancement, or clicks enabled Skip once after the same item settles. These items do not count as Added. Other alerts remain visible and pause automation. An alert already open before the script loads still needs manual OK.
